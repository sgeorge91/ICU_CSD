    ...
    for filename in z.namelist():
        if not os.path.isdir(filename):
            ...
            if(len(temp)>=4)and(temp[0]!='patient'):
                ...
                continue
            if(ID in uid):
                if(temp[len(temp)-1]=='ABF.csv'):#read the file
                    nid=ID
                    ...
                    with z.open(filename) as f:
                        if((trit-text).total_seconds()>60*120 and (float(exsc)>2.5)):
                            ...
                            for line in f:
                                ...
                                if((t>text) and (t<trit) and (temp2[0]=='ARTm')):
                                    timearr.append(t)
...
df.to_csv('DT_Coh2_ABF_Taus_60min_Stratified_v1.dat',sep='\t')
